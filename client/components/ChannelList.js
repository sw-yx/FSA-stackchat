      ...
      <ul>
        <li>
          <NavLink to={RANDOM_CHANNEL} activeClassName="active">
            ...
            <span className="badge">{store.getState().messages.filter(el => el.channelId === 1).length}</span>
          </NavLink>
        </li>
        <li>
          <NavLink to={GENERAL_CHANNEL} activeClassName="active">
            <span># generally_speaking</span>
            <span className="badge">{store.getState().messages.filter(el => el.channelId === 2).length}</span>
          </NavLink>
        </li>
        <li>
          <NavLink to={DOGS_CHANNEL} activeClassName="active">
            <span># dogs_of_fullstack</span>
            <span className="badge">{store.getState().messages.filter(el => el.channelId === 3).length}</span>
          </NavLink>
        </li>
        <li>
          <NavLink to={LUNCH_CHANNEL} activeClassName="active">
            <span># lunch_planning</span>
            <span className="badge">{store.getState().messages.filter(el => el.channelId === 4).length}</span>
          </NavLink>
        </li>
      </ul>
    );
